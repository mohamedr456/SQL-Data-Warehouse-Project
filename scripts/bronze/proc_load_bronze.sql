...
-- Stored Procedure: Load Bronze Layer (Source {CRM , ERP} ==>> Bronze) --
...
To execute this Stored Procedure Run the following Line:
...
BEGIN
	...
	BEGIN TRY
		...
		WITH (
			FIRSTROW = 2, -- To escape the first Row in the CSV file, which is not an actual data but column's headers.
			FIELDTERMINATOR = ',', -- Define the delimeter betwwen columns.
			TABLOCK
		);
